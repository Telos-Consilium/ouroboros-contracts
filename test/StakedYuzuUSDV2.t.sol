...
contract StakedYuzuUSDV2Test is StakedYuzuUSDTest, IStakedYuzuUSDV2Definitions {
    ...
    function test_WithdrawWithSlippage_Revert_RedeemedMoreThanMaxShares() public {
        ...
        uint256 shares = styz2.previewWithdraw(assets);

        vm.prank(user1);
        vm.expectRevert(abi.encodeWithSelector(RedeemedMoreThanMaxShares.selector, shares, maxShares));
        styz2.withdrawWithSlippage(assets, user1, user1, maxShares);
    }
    ...
    function test_RedeemWithSlippage_Revert_WithdrewLessThanMinAssets() public {
        ...
        uint256 assets = styz2.previewRedeem(shares);

        vm.prank(user1);
        vm.expectRevert(abi.encodeWithSelector(WithdrewLessThanMinAssets.selector, assets, minAssets));
        styz2.redeemWithSlippage(shares, user1, user1, minAssets);
    }
